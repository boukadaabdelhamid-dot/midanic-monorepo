...
async function runProductTypesMigration() {
  try {
    await pool.query(`ALTER TABLE "product_types" ADD COLUMN IF NOT EXISTS "image_url" text;`);
    logger.info("product_types.image_url column ready.");
  } catch (err) {
    logger.warn({ err }, "product_types migration skipped (non-fatal)");
  }
}
