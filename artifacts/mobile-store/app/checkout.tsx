  ...
  const handlePlaceOrder = () => {
    ...
    createOrder.mutate(
      ...
      },
      {
        ...
        onError: () => {
          Alert.alert(t("خطأ", "Error"), t("فشل في إنشاء الطلب", "Failed to place order"));
        },
      }
    );
  };
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      >
        ...
        <View style={[styles.field, styles.fieldMulti, { borderColor: colors.border, backgroundColor: colors.card }]}>
          <Feather name="map-pin" size={16} color={colors.mutedForeground} style={{ marginTop: 2 }} />
          <TextInput
            style={[styles.input, styles.inputMulti, { color: colors.foreground, textAlign: isRTL ? "right" : "left" }]}
          ...
          {cartItems.map((item) => (
            <View key={item.id} style={styles.summaryRow}>
              <Text style={[styles.sumName, { color: colors.foreground }]} numberOfLines={1}>
                {t(item.product?.nameAr ?? "", item.product?.nameEn ?? "")} ×{item.quantity}
              </Text>
...
const styles = StyleSheet.create({
  ...
  sectionLabel: { fontSize: 12, fontFamily: "Inter_600SemiBold", letterSpacing: 0.5, textTransform: "uppercase", marginBottom: 2 },
  field: { flexDirection: "row", alignItems: "center", borderRadius: 12, borderWidth: 1, paddingHorizontal: 14, paddingVertical: 12, gap: 10 },
  ...
  sumName: { fontSize: 14, fontFamily: "Inter_400Regular", flex: 1, marginRight: 8 },
