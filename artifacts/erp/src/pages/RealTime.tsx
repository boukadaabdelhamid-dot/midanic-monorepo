  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Mise à jour automatique toutes les 10 secondes · آخر تحديث:{" "}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          {
            label: "Commandes aujourd'hui", labelAr: "طلبات اليوم",
            value: todayOrders.length,
            icon: ShoppingCart, color: "text-blue-600", bg: "bg-blue-50", border: "border-blue-100"
          },
          {
            label: "CA du jour", labelAr: "رقم أعمال اليوم",
            value: `دج ${todayRevenue.toLocaleString("fr-DZ", { minimumFractionDigits: 0 })}`,
            icon: TrendingUp, color: "text-emerald-600", bg: "bg-emerald-50", border: "border-emerald-100"
          },
          {
            label: "En attente", labelAr: "في الانتظار",
            value: pendingCount,
            icon: Clock, color: "text-amber-600", bg: "bg-amber-50", border: "border-amber-100"
          },
          {
            label: "Total clients", labelAr: "إجمالي العملاء",
            value: analytics?.totalOrders ?? 0,
            icon: Users, color: "text-purple-600", bg: "bg-purple-50", border: "border-purple-100"
          },
        ].map(({ label, labelAr, value, icon: Icon, color, bg, border }) => (
          <Card key={label} className={`border-2 ${border} ${bg}`}>
            <CardContent className="p-4">
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-xs text-muted-foreground">{label}</p>
                  <p className="text-xs text-muted-foreground/70" dir="rtl">{labelAr}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="flex gap-3">
          {[
            { label: "En attente", labelAr: "انتظار", count: pendingCount, color: "bg-amber-500" },
            { label: "En cours", labelAr: "جارٍ", count: processingCount, color: "bg-blue-500" },
            { label: "Livrées", labelAr: "مُسلَّمة", count: deliveredCount, color: "bg-emerald-500" },
          ].map(({ label, labelAr, count, color }) => (
            <Card key={label} className="flex-1 border shadow-sm text-center">
              <CardContent className="p-3">
                <div className={`w-10 h-10 rounded-full ${color} flex items-center justify-center mx-auto mb-2`}>
                  <span className="text-white font-bold text-sm">{count}</span>
                </div>
                <p className="text-xs font-medium text-foreground">{label}</p>
                <p className="text-xs text-muted-foreground" dir="rtl">{labelAr}</p>
              </CardContent>
            </Card>
          ))}
        </div>
          ...
          <div className="divide-y">
            {recentOrders.length === 0 ? (
              <div className="text-center py-10 text-muted-foreground">
                ...
                <p className="text-sm">Aucune activité / لا يوجد نشاط</p>
              </div>
            ) : (
              recentOrders.map((order) => {
                const s = STATUS_COLORS[order.status ?? "pending"] ?? STATUS_COLORS["pending"];
                const timeSince = order.createdAt
                ...
                return (
                  <div key={order.id} className="flex items-center gap-3 px-4 py-3 hover:bg-muted/30 transition-colors">
                    ...
                    <div className="text-right shrink-0">
                      <p className="text-sm font-bold text-primary">دج {parseFloat(order.totalAmount ?? "0").toLocaleString()}</p>
