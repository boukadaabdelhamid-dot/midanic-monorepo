...
import {
  Wallet, ArrowUpCircle, ArrowDownCircle, Plus, Minus, Pencil, Trash2,
  ShoppingCart, Search, Save, RotateCcw, Printer, X, Eye, EyeOff,
  Settings, Users, Barcode, Check,
} from "lucide-react";
...
type CartLine = {
  ...
  qtyBonus: number;
  pu: number;
  reduction: number; // discount in DZD per line
};
...
export default function Caisse() {
  const [tab, setTab] = useState<"vente" | "caisse">("vente");
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold flex items-center gap-2">
          ...
          Ventes & Caisse / المبيعات والصندوق
        </h1>
      </div>

      <Tabs value={tab} onValueChange={(v) => setTab(v as "vente" | "caisse")} className="w-full">
        <TabsList className="grid w-full max-w-md grid-cols-2">
          <TabsTrigger value="vente" data-testid="tab-vente">
            <ShoppingCart className="h-4 w-4 mr-2" />
            Vente / بيع
          </TabsTrigger>
          <TabsTrigger value="caisse" data-testid="tab-caisse">
...
function PosView() {
  ...
  // Versement (payment received now)
  ...
  function handlePaymentConfirm(opts: { mode: "comptant" | "terme"; cloture: boolean; impression: boolean }) {
    ...
    const customerAddress = "Vente comptoir";

    // Note: backend uses authoritative product prices; line-level overrides
    // (pu, reduction, qtyBonus) are visual-only and not sent in this payload.
    // Server records the income transaction automatically on successful order.
    createOrder.mutate(
      { data: { customerName, customerPhone, customerAddress, items } },
      {
        onSuccess: (order) => {
          // Refresh ledger so the legacy Caisse tab reflects the new sale
          ...
          alert(
            ...
            `Montant facturé par le serveur: voir l'historique des commandes.`
          );
        },
        ...
        <CardContent className="p-4 space-y-3">
          ...
          <div className="grid grid-cols-[1fr_140px] gap-3">
            <div className="relative">
              <Label className="text-[11px] text-muted-foreground absolute -top-2 left-3 bg-white px-1 z-10">
                Code
              </Label>
              <Input
                ref={codeRef}
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter") tryAddByCode(code, qty);
                }}
                placeholder=""
            ...
            <Table>
              <TableHeader className="bg-slate-50">
                <TableRow>
                  ...
                  {showMontant && (
                    <TableHead className="text-right font-semibold w-28">Montant</TableHead>
                  )}
                  <TableHead className="w-10 text-center">
                    <Button
                      ...
                      size="icon"
                      className="h-7 w-7"
                      onClick={() => setShowMontant((v) => !v)}
                      aria-label="Toggle Montant"
                    >
                        ...
                        <TableCell className="text-center">
                          <div className="flex gap-1 justify-end">
                            <Button
                              ...
                              aria-label="Modifier"
                              data-testid={`button-edit-line-${idx}`}
                            >
          ...
          <div className="flex items-center justify-between text-sm">
            <span className="text-muted-foreground">
              {lines.length} articles / {totalArticles} (KG)
            </span>
            <Button
              size="sm"
              variant="outline"
          ...
          {/* Bottom toolbar (SellSabeel-style) */}
          ...
          <CardContent className="p-3 space-y-2">
            <div className="flex items-center justify-between">
              <Button
                size="sm"
                variant="outline"
                className="h-7 px-2 rounded-full text-emerald-600 border-emerald-200"
              >
                A+
              </Button>
              <div className="flex items-center gap-1">
        ...
        <DialogContent className="max-w-sm">
            ...
            <div className="space-y-3 py-2">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Input
                    type="number"
                    min="1"
                    value={editLine.line.qty}
                    onChange={(e) =>
                      setEditLine({
                        ...editLine,
                        line: { ...editLine.line, qty: Math.max(1, parseInt(e.target.value) || 1) },
                      })
                    }
                    className="h-9"
                ...
                <div>
                  ...
                  <Input
                    type="number"
                    min="0"
                    value={editLine.line.qtyBonus}
                    onChange={(e) =>
                      setEditLine({
                        ...editLine,
                        line: { ...editLine.line, qtyBonus: Math.max(0, parseInt(e.target.value) || 0) },
                      })
                    }
                    className="h-9"
                  />
                ...
                <div>
                  ...
                  <Input
                    type="number"
                    step="0.01"
                    value={editLine.line.pu}
                    onChange={(e) =>
                      setEditLine({
                        ...editLine,
                        line: { ...editLine.line, pu: parseFloat(e.target.value) || 0 },
                      })
                    }
                    className="h-9"
                  />
                ...
                <div>
                  ...
                  <Input
                    ...
                    step="0.01"
                    min="0"
                    value={editLine.line.reduction}
                    onChange={(e) =>
                      setEditLine({
                        ...editLine,
                        line: { ...editLine.line, reduction: Math.max(0, parseFloat(e.target.value) || 0) },
                      })
                    }
                    className="h-9"
                  />
              ...
              <div className="bg-slate-50 rounded p-2 text-sm flex justify-between">
                ...
                <span className="font-bold">
                  دج {fmt(editLine.line.pu * editLine.line.qty - editLine.line.reduction)}
                </span>
              </div>
            </div>
          )}
          <DialogFooter>
            ...
            <Button
              onClick={() => {
                if (editLine) {
                  updateLine(editLine.idx, editLine.line);
                  setEditLine(null);
                }
              }}
  ...
  return (
    <>
      <Button
        ...
        aria-label="Choisir client"
        data-testid="button-pick-client"
      >
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        ...
        <Input
          ...
          className="h-11"
          autoFocus
          data-testid="input-picker-filter"
        />
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map((p) => (
              ...
              >
                ...
                <div className="text-sm font-bold text-slate-600 ml-2">
                  {p.stock}
                </div>
              </button>
            ))
      ...
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="text-center">
            Règlement de la commande / تسوية الطلبية
          </DialogTitle>
        </DialogHeader>
        <div className="space-y-3 py-2">
          {/* Toggles row */}
          ...
          <div className="space-y-1.5 text-sm">
            <div className="flex justify-between">
              ...
              <span className={soldeClient < 0 ? "text-red-600 font-semibold" : "font-semibold"}>
                {fmt(soldeClient)}
              </span>
            </div>
            <div className="flex justify-between">
          ...
          <div className="relative">
            <Input
              ...
              onChange={(e) => {
                ...
                setVersement(parseFloat(e.target.value) || 0);
              }}
              placeholder="0,00"
          ...
          <div className="flex justify-between text-sm">
            ...
            <span className="font-semibold">{fmt(versementMin)}</span>
          </div>
...
// ============================================================================
// Cash Flow View (existing functionality preserved)
// ============================================================================
function CashFlowView() {
  ...
  const totalIn = todayTx.filter((t) => t.type === "income")
    .reduce((s, t) => s + parseFloat(t.amount ?? "0"), 0);
  const totalOut = todayTx.filter((t) => t.type === "expense")
    .reduce((s, t) => s + parseFloat(t.amount ?? "0"), 0);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        >
          {sessionOpen ? "Fermer la session" : "Ouvrir la session"}
        </Button>
      </div>
        ...
        <Card className="border-2 border-emerald-100 bg-emerald-50/50">
          <CardContent className="p-5 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-emerald-600">دج {fmt(totalIn)}</p>
            </div>
          </CardContent>
        </Card>
        <Card className="border-2 border-red-100 bg-red-50/50">
          <CardContent className="p-5 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-red-500">دج {fmt(totalOut)}</p>
            </div>
          </CardContent>
        </Card>
      ...
      <Card className="border shadow-sm">
        <CardHeader className="pb-2">
          <CardTitle className="text-base">Mouvements du jour ({todayTx.length})</CardTitle>
        </CardHeader>
                  ...
                  [...todayTx].reverse().map((t) => (
                    <TableRow key={t.id}>
                      ...
                      <TableCell className={`text-right font-bold ${t.type === "income" ? "text-emerald-600" : "text-red-600"}`}>
                        {t.type === "income" ? "+" : "-"} دج {fmt(parseFloat(t.amount ?? "0"))}
                      </TableCell>
                    </TableRow>
                  ))
                )}
        ...
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle className={openDialog === "in" ? "text-emerald-600" : "text-red-600"}>
              {openDialog === "in" ? "✚ Entrée de caisse" : "✖ Sortie de caisse"}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-3 py-2">
            <div>
